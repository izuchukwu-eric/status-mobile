...
(fx/defn navigate-to-chat
  ...
  (fx/merge cofx
            ...
            (fn [{:keys [db]}]
              ...
              ;;  TODO: Remove dispatch later and uncomment line 264 when tester has done testing
...
(fx/defn navigate-to-chat-nav2
  ...
  (fx/merge cofx
            {:db (assoc db :current-chat-id chat-id)
             ;;  TODO: Remove dispatch later  and uncomment line 278 when tester has done testing
             :dispatch-later [{:ms 5000
                               :dispatch [:chat.ui/preload-chat-data chat-id]}]}
            (offload-messages chat-id)
            ;; (preload-chat-data chat-id)
